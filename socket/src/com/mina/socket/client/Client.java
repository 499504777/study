...
public class Client {
	...
	public void start(){
		...
		form.getConButton().addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if("".equals(ip) || ip == null){
					form.setContent("请输入ip地址！");return;
				}
				if("".equals(port) || port == null){
					form.setContent("请输入连接端口！");return;
				}
				if("".equals(realName) || realName == null){
					form.setContent("请输入用户名！");return;
				}
	...
	private void doJob(){
		try{
			...
			//监听输入框回车事件
			form.getTextField().addActionListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					String message = form.getSendMessage();
					String msg = realName+":"+message;
